"""Tests for the CNN-LSTM Trend Classifier algorithm."""
...
from daytrader.algorithms.builtin.cnn_lstm_trend import CNNLSTMTrendAlgorithm
...
def test_manifest():
    algo = CNNLSTMTrendAlgorithm()
    m = algo.manifest
    assert m.id == "cnn_lstm_trend"
    assert m.name == "CNN-LSTM Trend"
    assert "crypto" in m.asset_classes
    assert "equities" in m.asset_classes


def test_warmup_bars():
    algo = CNNLSTMTrendAlgorithm(lookback=50)
...
def test_on_bar_before_training_returns_none():
    algo = CNNLSTMTrendAlgorithm(auto_train=False)
...
@pytest.mark.asyncio
async def test_cnn_lstm_end_to_end_backtest():
    algo = CNNLSTMTrendAlgorithm(
...
def test_registry_includes_cnn_lstm():
    AlgorithmRegistry.clear()
    AlgorithmRegistry.auto_register()
    assert "cnn_lstm_trend" in AlgorithmRegistry.available()
    algo = AlgorithmRegistry.get("cnn_lstm_trend")
    assert algo.manifest.id == "cnn_lstm_trend"
