"""Tests for the LSTM Trend Classifier algorithm."""
...
def test_manifest():
    algo = LSTMTrendAlgorithm()
    m = algo.manifest
    assert m.id == "lstm_trend"
    assert m.name == "LSTM Trend"
    assert "crypto" in m.asset_classes
    assert "equities" in m.asset_classes


def test_warmup_bars():
    algo = LSTMTrendAlgorithm(lookback=50)
    assert algo.warmup_bars() == 50

    algo2 = LSTMTrendAlgorithm(lookback=100)
...
def test_train_sets_is_trained():
    algo = LSTMTrendAlgorithm(lookback=20, sequence_length=10, epochs=5, auto_train=False)
...
def test_train_insufficient_data():
    algo = LSTMTrendAlgorithm(lookback=20, sequence_length=10, auto_train=False)
...
def test_on_bar_before_training_returns_none():
    algo = LSTMTrendAlgorithm(auto_train=False)
...
@pytest.mark.asyncio
async def test_lstm_end_to_end_backtest():
    algo = LSTMTrendAlgorithm(lookback=30, sequence_length=10, epochs=5, auto_train=True)
...
def test_registry_includes_lstm():
    AlgorithmRegistry.clear()
    AlgorithmRegistry.auto_register()
    assert "lstm_trend" in AlgorithmRegistry.available()
    algo = AlgorithmRegistry.get("lstm_trend")
    assert algo.manifest.id == "lstm_trend"
